...
import { Brain, Search, Trash2, Sparkles, User, BookOpen, Lightbulb, Heart } from 'lucide-react'
      ...
      <div className="flex justify-between items-center border-b border-codebox-border pb-4">
        ...
        <div className="flex gap-2">
          <button className="btn-primary flex items-center gap-2" onClick={handleLearn}>
            <Sparkles size={14} /> Learn from sessions
          </button>
          <button className="bg-codebox-input border border-codebox-border text-codebox-secondary px-3.5 py-2 rounded-lg font-medium cursor-pointer text-xs hover:text-codebox-red" onClick={handlePrune}>
            <Trash2 size={14} />
          </button>
        </div>
      </div>
        ...
        <div className="flex-1 flex flex-col gap-5">
          ...
          {filtered.length === 0 ? (
            <div className="py-16 text-center text-codebox-muted border border-dashed border-codebox-border rounded-xl">
              ...
              <p className="text-xs mt-1">Eburon learns from your sessions over time. Click "Learn from sessions" to seed initial memories.</p>
            </div>
          ) : (
